...
import { ExternalLink, CheckCircle2, Calendar, Target, Lightbulb, Layers } from "lucide-react";
  ...
  return (
    <main className="pt-20">
          ...
          <div className="grid grid-cols-1 lg:grid-cols-2 gap-16 items-center">
            <MotionSection>
              ...
              <p className="text-lg text-[#4B5563] max-w-lg leading-relaxed">
                Salesforce work I&apos;ve shipped. Business outcomes, not feature lists — click any project for the
                full story.
              </p>
            </MotionSection>
            ...
            <motion.div
              ...
              transition={{ delay: 0.3, duration: 0.8, ease: EASE_PREMIUM }}
            ...
            {projects.map((project) => (
              <StaggerItem key={project.id}>
                <motion.div whileHover={{ y: -4 }} transition={{ duration: 0.3, ease: EASE_PREMIUM }}>
                  <Card
                    className="rounded-2xl border-[#E5E7EB] hover:shadow-lg hover:border-[#CB9135]/30 cursor-pointer group h-full flex flex-col overflow-hidden"
                    ...
                    <div className="p-7 flex-grow flex flex-col">
                      <div className="flex items-center justify-between mb-4">
                        <div className="flex items-center gap-2">
                          <Badge variant="secondary" className="text-xs bg-[#F1F1EF] text-[#4B5563] hover:bg-[#F1F1EF]">
                            {project.category}
                          </Badge>
                          {project.industry && (
                            <Badge variant="outline" className="text-xs border-[#CB9135]/30 text-[#CB9135] hover:bg-transparent">
                              {project.industry}
                            </Badge>
                          )}
                        </div>
                      ...
                      <h3 className="text-xl font-semibold text-[#2F2E2E] mb-1 tracking-tight">{project.title}</h3>
      ...
      <Dialog open={!!selectedProject} onOpenChange={() => setSelectedProject(null)}>
        <DialogContent className="max-w-2xl max-h-[85vh] overflow-y-auto bg-white pt-10 rounded-2xl">
          {selectedProject && (
            <>
              <DialogHeader>
                <div className="flex items-center gap-2 mb-3">
                  <Badge variant="secondary" className="w-fit text-xs bg-[#F1F1EF] text-[#4B5563] hover:bg-[#F1F1EF]">
                    {selectedProject.category}
                  </Badge>
                  {selectedProject.industry && (
                    <Badge
                      variant="outline"
                      className="w-fit text-xs border-[#CB9135]/30 text-[#CB9135] hover:bg-transparent"
                    >
                      {selectedProject.industry}
                    </Badge>
                  )}
                </div>
                <DialogTitle className="text-2xl font-bold text-[#2F2E2E] tracking-tight">
                ...
                <div className="grid grid-cols-2 gap-4">
                  {selectedProject.impact.map((imp, idx) => (
                    <div key={idx} className="bg-[#F1F1EF] rounded-lg p-5 text-center">
                      <p className="text-2xl font-bold text-[#2F2E2E] tabular-nums tracking-tight">{imp.metric}</p>
        ...
        <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 text-center relative z-10">
          <MotionSection>
            ...
            <p className="text-gray-400 mb-10 max-w-xl mx-auto leading-relaxed">
              Let&apos;s discuss what&apos;s not working and map out an approach — free, no commitment.
            </p>
